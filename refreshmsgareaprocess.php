...
if (isset($_SESSION["u"])) {
    ...
    $senderrs = Database::search("SELECT * FROM `chat` WHERE (`from`='$sender' AND `to`='$recever') OR (`from`='$recever' AND `to`='$sender') ORDER BY `date` ASC");
    ...
    if ($img->num_rows == 0) $img = "resources//profile.jpg";
    else {
        $img = $img->fetch_assoc();
        $img = $img['code'];
    }
